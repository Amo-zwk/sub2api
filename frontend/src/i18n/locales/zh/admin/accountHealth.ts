export default {
  accountHealth: {
    ...
    actions: { refresh: '刷新', run: '立即巡检', save: '应用设置' },
    summary: {
      total: '账号总数', eligible: '调度可用', healthy: '健康', degraded: '异常重试',
    ...
    settings: {
      ...
      authInterval: '认证重试间隔（秒）', entitlementInterval: '额度重试间隔（秒）',
      permissionInterval: '权限重试间隔（秒）', reconcileInterval: '状态对账间隔（秒）', maxProbeQps: '巡检 QPS 上限',
    ...
    states: {
      unknown: '未巡检', healthy: '健康', probation: '观察中', transient_error: '临时错误',
      auth_quarantine: '认证隔离', entitlement_quarantine: '额度隔离', permission_quarantine: '权限隔离',
      degraded: '异常', recovering: '恢复中', blocked: '已隔离'
    },
